...
class VideoSettings:
    ...
    checkpoint: Optional[str] = "dreamshaper_8.safetensors"
    format: VideoFormat = VideoFormat.MP4
    interpolate: bool = False  # Use RIFE to double frames

    # v2.5.0: New fields for advanced models
    variant: Optional[str] = None       # Model variant (e.g., "1.3b", "14b", "distilled")
    upscale: bool = False               # Enable super-resolution (Hunyuan 1.5)
    shift: Optional[float] = None       # ModelSamplingSD3 shift override
...
VIDEO_PRESETS: Dict[str, VideoSettings] = {
    ...
    "hunyuan15_fast": VideoSettings(
        ...
        width=848, height=480,
        frames=81, fps=24,
        steps=6, cfg=1.0,  # Distilled model uses CFG=1
        variant="distilled",
        checkpoint=None,
    ),
    ...
    "ltx_quick": VideoSettings(
        ...
        frames=97, fps=24,
        steps=20, cfg=3.0,  # LTXV uses low CFG
        checkpoint=None,
    ),
    ...
    "wan_fast": VideoSettings(
        ...
        width=640, height=640,
        frames=81, fps=16,
        steps=4, cfg=1.0,  # 4-step with LoRA
...
class VideoWorkflowBuilder:
    ...
    def _build_animatediff(
        ...
        motion_module = "v3_sd15_mm.ckpt" if settings.model == VideoModel.ANIMATEDIFF_V3 else "mm_sd_v15_v2.ckpt"
    ...
    def _build_svd(
        ...
        model_name = "svd_xt.safetensors" if settings.model == VideoModel.SVD_XT else "svd.safetensors"
...
_builder: Optional[VideoWorkflowBuilder] = None
    ...
    **overrides
    ...
    if overrides:
        ...
        settings = VideoSettings(
            ...
            motion_style=MotionStyle(settings_dict.get("motion_style", settings.motion_style.value)),
...
def get_video_preset(name: str) -> Optional[VideoSettings]:
...
def get_recommended_preset(
    ...
    if vram_gb < 8:
        ...
        candidates = ["ltx_standard", "ltx_quality", "wan_14b", "mochi_short",
                     "standard", "quality", "cinematic", "portrait", "action"]

    elif vram_gb < 24:
        # 16-24GB: Hunyuan 1.5 720p, LTX quality, Wan quality
        candidates = ["hunyuan15_720p", "hunyuan15_fast", "ltx_quality",
                     "wan_quality", "wan_fast", "quality", "cinematic",
                     "svd_short", "svd_long", "cogvideo"]
    ...
    if quality == "fast":
        ...
        best_order = ["hunyuan15_quality", "mochi", "hunyuan15_720p", "wan_quality",
                     "ltx_quality", "hunyuan", "quality", "cinematic"]
